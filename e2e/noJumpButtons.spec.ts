...
test.describe('No Confirm/Undo jump buttons', () => {
  test('local mode does not show Confirm jump or Undo jump buttons during a multi-jump', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("Confirm jump")')).not.toBeVisible({ timeout: 1000 });
  ...
  test('multiplayer mode does not show Confirm jump or Undo jump buttons', async ({ browser }) => {
    ...
    await expect(alicePage.locator('button:has-text("Confirm jump")')).not.toBeVisible({ timeout: 1000 });
    await expect(alicePage.locator('button:has-text("Undo jump")')).not.toBeVisible({ timeout: 1000 });
    await expect(bobPage.locator('button:has-text("Confirm jump")')).not.toBeVisible({ timeout: 1000 });
    await expect(bobPage.locator('button:has-text("Undo jump")')).not.toBeVisible({ timeout: 1000 });
